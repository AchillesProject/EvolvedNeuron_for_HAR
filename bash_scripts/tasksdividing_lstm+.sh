...
python_scripts_path='../pythons/lstm_plus/run/*.py'
...
for python_script in "${python_scripts[@]}"; do
    ...
    python_script_log="../pythons/lstm_plus/logs/log_${python_script_name}.txt";
    echo "Running ${python_script_name} script and logging at ${python_script_log}.";
    `taskset -c "${cpu_index}-$((${cpu_index}+10))" python "${python_script}" &>"${python_script_log}" &!`;
    cpu_index=$(($cpu_index+10));
    echo "${cpu_index}"
done
