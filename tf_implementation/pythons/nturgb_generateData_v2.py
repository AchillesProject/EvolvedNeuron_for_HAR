                    ...
                    for nbody in range(nbodies): 
                        ...
                        if skel_body in mat:
                            for i in range(math.ceil(nframe/seq_no)):
                                diff_no = 0 if (i+1)*seq_no < nframe else (i+1)*seq_no - nframe
                                new_row = mat[skel_body][i*seq_no-diff_no:(i+1)*seq_no-diff_no].reshape(seq_no, -1)
                                new_row = np.expand_dims(np.concatenate((new_row, np.tile(activity_arr, new_row.shape[0]).T), axis=-1), axis=0)
                                fileset = new_row if fileset.shape[0] == 0 else np.concatenate((fileset, new_row), axis=0)
