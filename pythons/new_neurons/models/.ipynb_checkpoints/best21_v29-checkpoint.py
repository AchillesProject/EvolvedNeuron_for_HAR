...
class RNN_plus_v1_29_cell(tf.keras.layers.LSTMCell):
    def __init__(self, units, kernel_initializer='glorot_uniform', recurrent_initializer='orthogonal', bias_initializer='zeros', dropout=0., recurrent_dropout=0., use_bias=True, **kwargs):
        ...
        super(RNN_plus_v1_29_cell, self).__init__(units, **kwargs)
        self.units = units
...
def rnn_plus_model(noInput, noOutput, timestep):
    ...
    model.add(tf.keras.layers.RNN(cell=RNN_plus_v1_29_cell(units=hyperparams['noUnits']), input_shape=[timestep, noInput], unroll=False, name='RNNp_layer', dtype=DTYPE))
