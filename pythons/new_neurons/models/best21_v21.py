...
class RNN_plus_v1_21_cell(tf.keras.layers.LSTMCell):
    def __init__(self, units, kernel_initializer='glorot_uniform', recurrent_initializer='orthogonal', bias_initializer='zeros', dropout=0., recurrent_dropout=0., use_bias=True, **kwargs):
        ...
        super(RNN_plus_v1_21_cell, self).__init__(units, **kwargs)
        self.units = units
    ...
    def call(self, inputs, states, training=None):
        ...
        z2 = tf.nn.tanh(srelu(tf.nn.tanh(w_aux[1]*op2 + w_aux[2]*state0 + w_aux[3]))) #remove 2 tanh and w_aux[2]*state3 -> w_aux[2]*state0
        z3 = tf.nn.tanh(tf.nn.relu(inputs_2))
        z  = z1 - (z2 + z3)
        output = prev_output - (z - state0) #(z - state1)*z -> (z - state0)
        f = w_aux[4]*z + op0
...
def rnn_plus_model(noInput, noOutput, timestep):
    ...
    model.add(tf.keras.layers.RNN(cell=RNN_plus_v1_21_cell(units=hyperparams['noUnits']), input_shape=[timestep, noInput], unroll=False, name='RNNp_layer', dtype=DTYPE))
