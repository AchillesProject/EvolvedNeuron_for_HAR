...
class RNN_plus_v1_25_cell(tf.keras.layers.LSTMCell):
    ...
    def call(self, inputs, states, training=None):
        ...
        z2 = tf.nn.tanh(w_aux[1]*op2 + w_aux[2]*state2 + w_aux[3]) #remove 2 tanh and w_aux[2]*state3 -> w_aux[2]*state2 & remove srelu
...
def rnn_plus_model(noInput, noOutput, timestep):
    ...
    model.add(tf.keras.layers.RNN(cell=RNN_plus_v1_25_cell(units=hyperparams['noUnits']), input_shape=[timestep, noInput], unroll=False, name='RNNp_layer', dtype=DTYPE))
